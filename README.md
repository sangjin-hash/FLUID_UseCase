...
## Goal
멀티 서피스 관련 기존의 연구들 중, 제시된 대표적인 접근법은 다중 기기 환경에 특화된 모바일 운영체제를 새롭게 설계하는 것이다. 하지만 이는 모바일 운영체제 내부의 사용자 인터페이스 관리/렌더링에 관련된 핵심 요소들을 대폭 수정해야 하기 때문에, 모바일 운영체제 시장을 선점하고 있는 Google, Apple 등 여러 주요 글로벌 기업과의 협력이 반드시 요구된다. 이러한 제약 사항은 멀티 서피스 기술의 적용 가능성을 심각하게 저해한다.

이러한 한계점에 착안하여, **본 연구에서는 싱글 서피스를 위해 만들어진 기존 앱을 멀티 서피스 앱으로 자동 변환해주는 프레임워크를 설계하는 것을 목표로 하고자 한다.** 이는 개별 모바일 앱이 자신의 일부 UI 요소를 여러 기기로 분산시킬 수 있도록 앱 기능을 확장해줌으로써, 사용자의 유연한 멀티 서피스 활용을 지원한다. **앱 기능 확장은 앱 레벨에서 UI 분산을 위해 필요한 코드 로직을 추가하는 방식으로 이루어지기 때문에, 모바일 운영체제에 대한 수정을 요구하지 않으며,** 그 기술 적용이 용이하다.
</br>
</br>
